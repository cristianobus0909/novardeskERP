...
export function ClienteSelector() {
  ...
  if (selectedCliente) {
    return (
      <div className="d-flex align-center justify-between" style={{ background: 'var(--bg-tertiary)', padding: '12px', borderRadius: '8px', border: '1px solid var(--border-color)', marginBottom: '16px' }}>
        <div>
          <div style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>Cliente Seleccionado</div>
          <div className="font-bold">{selectedCliente.razon_social}</div>
  ...
  return (
    <div className="relative" style={{ marginBottom: '16px' }}>
      {!isCreating ? (
        <div className="d-flex gap-sm align-start">
          <div className="flex-1">
            ...
            {search && filtered && filtered.length > 0 && (
              <div className="absolute" style={{ top: '100%', left: 0, right: 0, background: 'var(--bg-primary)', border: '1px solid var(--border-color)', borderRadius: '8px', zIndex: 10, marginTop: '4px', boxShadow: '0 4px 12px rgba(0, 0, 0, 0.1)' }}>
                {filtered.map((c: any) => (
                  <div 
                    key={c.id} 
            ...
            {search && filtered && filtered.length === 0 && (
              <div className="absolute text-center" style={{ top: '100%', left: 0, right: 0, background: 'var(--bg-primary)', border: '1px solid var(--border-color)', borderRadius: '8px', zIndex: 10, marginTop: '4px', padding: '12px', fontSize: '13px' }}>
          ...
          <button 
            ...
            className="btn-secondary p-0 d-flex align-center justify-center" style={{ marginTop: '22px', width: '42px', height: '42px', flexShrink: 0, borderRadius: '8px' }}
            title="Nuevo Cliente"
          >
        ...
        <div style={{ background: 'var(--bg-tertiary)', padding: '12px', borderRadius: '8px', border: '1px dashed var(--border-color)' }}>
          <div className="d-flex justify-between" style={{ marginBottom: '8px' }}>
            <span className="font-bold" style={{ fontSize: '14px' }}>Alta Rápida de Cliente</span>
            <button type="button" onClick={() => setIsCreating(false)} style={{ background: 'none', border: 'none', cursor: 'pointer' }}>&times;</button>
          </div>
          <div className="gap-sm" style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr' }}>
