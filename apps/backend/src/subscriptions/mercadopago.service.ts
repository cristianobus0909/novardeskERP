...
export class MercadoPagoService {
  ...
  async createSubscriptionPreference(tenantId: number, email: string, tier: string = 'PREMIUM') {
    this.logger.log(`Creando preferencia de suscripción para Tenant ${tenantId} (${email}), Plan: ${tier}`);

    if (this.isMockMode()) {
      this.logger.log('Modo simulador: Retornando URL de checkout simulada');
      return {
        id: `sub_active_${tenantId}_${Math.floor(Math.random() * 1000)}`,
        init_point: `http://localhost:3001/mock-checkout?preapproval_id=sub_active_${tenantId}_${Math.floor(Math.random() * 1000)}&tenant_id=${tenantId}&tier=${tier}`,
      };
    }

    let transaction_amount = 29000;
    if (tier === 'BASICO') transaction_amount = 19000;
    if (tier === 'FULL') transaction_amount = 44900;
    ...
    try {
      const response = await fetch('https://api.mercadopago.com/preapproval', {
        ...
        body: JSON.stringify({
          ...
          reason: `Suscripción NovarDesk ERP - Plan ${tier}`,
          auto_recurring: {
            frequency: 1,
          ...
          payer_email: email,
          external_reference: `${tenantId}_${tier}`,
          status: 'pending',
        }),
      });
  ...
  async getSubscriptionDetails(subscriptionId: string) {
    ...
    if (this.isMockMode() || subscriptionId.startsWith('sub_')) {
      ...
      // sub_[state]_[tenantId]_[tier]_[random]
      const state = parts[1] || 'active';
      const tenantId = parts[2] || '1';
      const tier = parts[3] || 'PREMIUM';
